...
Ключевое слово `IN` используется, когда неободимо применить оператор равенства для множеств 

#### ANY

Ключевое слово `ANY` используется, когда необходимо использовать <, =<, >, >= при сравнении множества

#### SOME

Ключевое слово `SOME` используется как алиас к `ANY`

#### ALL

Ключевое слово `ALL` это логическое И для вложенного запроса

#### EXISTS, NOT EXISTS

Эти ключевые слова проверяют количество возвращаемых строк 


#### JOIN

Каждому значению из 1 таблицы будет сопоставлено каждое значение из 2 таблицы
Содержимое полученной таблицы, можно фильтровать с помощью ON условия, которое работает во время сопоставления таблиц. 

#### Самообъединение таблиц

Это когда таблицу соединяют с самой собой.

##### LEFT JOIN

В результате будут выведены сопоставления для 1 таблицы из 2-й, а все отсутствующие сопоставления будут заменены null

##### RIGHT JOIN

В результате будут выведены сопоставления для 2  таблицы из 1-й, а все отсутствующие сопоставления будут заменены null


##### OUTER JOIN

MYSQL не поддерживает данное объединение таблиц
