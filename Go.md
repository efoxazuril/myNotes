...
Ключем может быть любой тип данных, для которого определена операция сравнения `==`
...
```
Только последний параметр функции может быть вариадитеским.
...
Для работы с ошибками есть стандартная библиотека [errors](https://golang.org/pkg/errors/)
...
Panic - это втроенная функцияб которая останавливает обычный поток управления. Лучше этим вообще не пользоваться - это дорогая операция.
